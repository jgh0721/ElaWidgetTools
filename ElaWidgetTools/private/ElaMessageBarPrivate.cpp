...
void ElaMessageBarPrivate::_drawSuccess(QPainter* painter)
{
    ...
    painter->drawText(_leftPadding, 0, q->width(), q->height(), Qt::AlignVCenter, QChar((int)ElaIconType::Check));
    // 时间进度条绘制
...
void ElaMessageBarPrivate::_drawError(QPainter* painter)
{
    ...
    painter->setFont(iconFont);
    painter->drawText(_leftPadding + 1, 0, q->width(), q->height(), Qt::AlignVCenter, QChar((int)ElaIconType::Xmark));
    // 时间进度条绘制
    QPainterPath clipPath;
    clipPath.addRoundedRect(foregroundRect, _borderRadius, _borderRadius);
    painter->setClipPath(clipPath);
