...
void ElaToolButtonStyle::_drawIndicator(QPainter* painter, const QStyleOptionToolButton* bopt, const QWidget* widget) const
{
    ...
    else if (bopt->features.testFlag(QStyleOptionToolButton::HasMenu))
    {
        ...
        int indicatorWidth = painter->fontMetrics().horizontalAdvance(QChar((int)ElaIconType::AngleDown));
        ...
        painter->drawText(expandIconRect, Qt::AlignCenter, QChar((int)ElaIconType::AngleDown));
        painter->restore();
    }
}
...
void ElaToolButtonStyle::_drawText(QPainter* painter, QRect contentRect, const QStyleOptionToolButton* bopt) const
{
    if (!bopt->text.isEmpty())
    {
        ...
        case Qt::ToolButtonTextOnly:
        {
            ...
            painter->drawText(contentRect, Qt::AlignLeft | Qt::AlignVCenter | Qt::TextShowMnemonic, bopt->text);
            break;
        }
        case Qt::ToolButtonTextBesideIcon:
        {
            painter->drawText(QRect(contentRect.x() + _contentMargin * 2 + bopt->iconSize.width(), contentRect.y(), contentRect.width() - bopt->iconSize.width(), contentRect.height()), Qt::AlignLeft | Qt::AlignVCenter | Qt::TextShowMnemonic, bopt->text);
            break;
        }
        case Qt::ToolButtonTextUnderIcon:
        {
            ...
            {
                contentRect.setLeft(contentRect.left() + _contentMargin);
                painter->drawText(contentRect, Qt::AlignBottom | Qt::AlignLeft | Qt::TextShowMnemonic, bopt->text);
            }
            else
            {
                painter->drawText(contentRect, Qt::AlignBottom | Qt::AlignHCenter | Qt::TextShowMnemonic, bopt->text);
            }
            break;
        }
        case Qt::ToolButtonFollowStyle:
