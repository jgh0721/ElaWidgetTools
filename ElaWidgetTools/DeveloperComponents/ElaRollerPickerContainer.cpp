...
void ElaRollerPickerContainer::paintEvent(QPaintEvent* event)
{
    ...
    else
    {
        ...
        // 确定
        painter.drawText(_overButtonRect, Qt::AlignCenter, QChar((int)ElaIconType::Check));
        // 取消
        painter.drawText(_cancelButtonRect, Qt::AlignCenter, QChar((int)ElaIconType::Xmark));
    }
    painter.restore();
}
