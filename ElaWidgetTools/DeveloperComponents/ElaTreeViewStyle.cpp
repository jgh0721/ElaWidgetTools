    ...
    case QStyle::PE_IndicatorBranch:
    {
        ...
        if (const QStyleOptionViewItem* vopt = qstyleoption_cast<const QStyleOptionViewItem*>(option))
        {
            if (vopt->state.testFlag(QStyle::State_Children))
            {
                ...
                painter->drawText(indicatorRect, Qt::AlignVCenter | Qt::AlignRight, vopt->state.testFlag(QStyle::State_Open) ? QChar((int)ElaIconType::AngleDown) : QChar((int)ElaIconType::AngleRight));
                painter->restore();
            }
        }
        return;
